...
async function main() {
  ...
  const modifyAction = {
    ...
    modifies: [{
      oid,
      order: { a: COIN, b: true, p: newPx, s: sz, r: false, t: { limit: { tif: "Gtc" } } },
    }],
  };

  res = await rpc("hl_buildModify", { action: modifyAction });
  sig = await signHash(res.result.hash);

  const modifyResult = await rpc("hl_sendModify", {
    action: modifyAction,
    nonce: res.result.nonce,
    signature: sig,
  });

  console.log(JSON.stringify(modifyResult.result.exchangeResponse, null, 2));
  console.log("\nOrder modified.");
}
